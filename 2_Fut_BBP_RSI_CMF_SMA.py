...
trade_time = now.replace(hour=13, minute=0, second=0, microsecond=0)
...
def getHistoricalData(symbol, resolution=15):
    ...
    for i in range(20, len(df)):
        if df['RSI'][i-1] <= df["REMA_9"][i-1] and df['RSI'][i] > df["REMA_9"][i]:
            df['BUY'].at[i] = 1
        if df['RSI'][i-1] >= df["REMA_9"][i-1] and df['RSI'][i] < df["REMA_9"][i]:
            df['SELL'].at[i] = 1
    # df.to_csv("E:/bhave/Desktop/2.csv", mode='a', header=True, index=False)
    return df


def checkSignal():
    start = pd.Timestamp.now(tz='Asia/Kolkata')
    nextscan = start+timedelta(minutes=TimeFrame)
    ...
    for symbol in SYMBOL_LIST:
        ...
        if candle_df is not None:
            ...
            if latest_candle['BUY'] == 1:
                ...
                SL = round(0.005 * latest_candle['close'], 1)
            ...
            if latest_candle['SELL'] == 1:
                ...
                print(f"Sell {symbol}\n{datetime.now()}")
    interval = int((nextscan-pd.Timestamp.now(tz='Asia/Kolkata')).seconds)
