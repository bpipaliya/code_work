...
def historicalData(symbol, resolution):
    df = fy_lib.historical_data(symbol, delta=100, resolution=resolution)
    columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
    df = pd.DataFrame(df['candles'], columns=columns)
    df['timestamp'] = (pd.to_datetime(df['timestamp'], unit='s').dt.tz_localize('utc').dt.tz_convert('Asia/Kolkata'))
    # df = df[['timestamp', 'open', 'high', 'low', 'close', 'volume']]
    start_date = (pd.Timestamp.now(tz='Asia/Kolkata').replace(second=0, microsecond=0).strftime("%Y-%m-%d"))
    df = df.iloc[::-1].set_index('timestamp').astype(float)
    # df = pd.DataFrame(df['candles']).iloc[::-1].set_index('timestamp').astype(float)
    # df = df[df.index >= start_date]
    df.index = pd.to_datetime(df.index)
    return df
tsla = historicalData(symbol= "NSE:ADANIPORTS21NOVFUT", resolution='D')
print(tsla)
...
tsla['st'], tsla['s_upt'], tsla['st_dt'] = get_supertrend(tsla['high'], tsla['low'], tsla['close'], 10, 2)
tsla = tsla[1:]
print(tsla.head())
...
profit_percentage = round(floor((total_investment_ret / investment_value) * 100), 2)
print(cl('Profit gained from the ST strategy by investing Rs.4,00,000/- in Company : {}'.format(total_investment_ret),
         attrs=['bold']))
